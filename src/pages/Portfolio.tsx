...
const GridItem: React.FC<{ item: PortfolioItem; onClick: () => void; isDarkMode: boolean }> = ({ item, onClick, isDarkMode }) => {
  const [isLoaded, setIsLoaded] = useState(false);

  return (
    <div 
      onClick={onClick}
      className={`group relative rounded-xl overflow-hidden cursor-pointer h-full w-full ${isDarkMode ? 'bg-stone-800' : 'bg-stone-200'} shadow-md hover:shadow-2xl transition-all duration-500`}
    >
      {/* Placeholder / Loader Background */}
      {!isLoaded && (
        <div className={`absolute inset-0 flex items-center justify-center ${isDarkMode ? 'bg-stone-800' : 'bg-stone-200'}`}>
...
const Portfolio: React.FC = () => {
  ...
  return (
    <div className={`min-h-screen ${isDarkMode ? 'bg-stone-950' : 'bg-stone-50'}`}>
      {/* Hero */}
      <div className={`${isDarkMode ? 'bg-stone-900' : 'bg-stone-900'} py-24 md:py-32 px-4 text-center relative overflow-hidden`}>
        <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] opacity-5"></div>
        <div className="max-w-7xl mx-auto text-center relative z-10">
            <span className={`text-gold-500 font-bold tracking-widest uppercase text-sm mb-4 block ${isDarkMode ? 'text-gold-400' : 'text-gold-500'}`}>Selected Projects</span>
          <h1 className={`font-serif text-5xl md:text-6xl font-bold ${isDarkMode ? 'text-white' : 'text-white'} mb-6`}>Our Work</h1>
          <p className={`${isDarkMode ? 'text-stone-300' : 'text-stone-400'} max-w-2xl mx-auto text-xl font-light`}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="flex flex-col md:flex-row justify-center items-center gap-4 mb-16">
            <div className={`flex items-center gap-2 ${isDarkMode ? 'text-stone-400' : 'text-stone-400'} md:hidden mb-2`}>
            ...
            {categories.map((cat) => (
                ...
                className={`px-6 py-2 rounded-full text-sm font-bold uppercase tracking-wider transition-all duration-300 border ${
                    ...
                    : `${isDarkMode ? 'bg-stone-800 text-stone-300 border-stone-700 hover:border-gold-500 hover:text-gold-500' : 'bg-white text-stone-500 border-stone-200 hover:border-gold-500 hover:text-stone-900'}`
                }`}
                >
                {cat}
                </button>
      ...
      {lightboxOpen && (
        ...
        >
          ...
          <button 
            ...
            className={`absolute top-6 right-6 ${isDarkMode ? 'text-stone-400 hover:text-white' : 'text-stone-400 hover:text-white'} transition-colors z-50 p-2 bg-white/10 rounded-full hover:bg-white/20`}
          >
            <X className="w-8 h-8" />
          </button>
          ...
          <div className="relative w-full h-full max-w-7xl mx-auto flex flex-col justify-center items-center p-4 md:p-12">
            ...
            <div className="absolute bottom-8 left-0 w-full text-center px-4 pointer-events-none">
                <div className={`transition-all duration-300 delay-100 ${isAnimating ? 'opacity-0 translate-y-4' : 'opacity-100 translate-y-0'}`}>
                    <h3 className={`text-2xl md:text-3xl font-serif font-bold ${isDarkMode ? 'text-white' : 'text-white'} mb-2`}>{filteredItems[currentIndex].title}</h3>
